...
export function ManageTaskForm() {
  ...
  const onSubmit: SubmitHandler<ManageFormInputs> = async (data: ManageFormInputs) => {
    if (!data.title) {
      return
    }
  ...
  return (
    <form className="flex flex-col items-center w-full gap-7 mb-7" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex flex-col gap-4 flex-1 ">
            <label className="flex flex-col gap-1">
              <div>
                ...
                <span className="text-xs text-gray-700 uppercase mb-1 ml-1">(optional)</span>
              </div>
